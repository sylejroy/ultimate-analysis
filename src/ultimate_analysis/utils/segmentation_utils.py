...
from ..gui.visualization import (
    create_unified_field_mask, calculate_field_contour, 
    draw_unified_field_mask, draw_field_contour, get_primary_field_color
)
...
def apply_segmentation_to_warped_frame(warped_frame: np.ndarray, 
                                     segmentation_results: List[Any],
                                     homography_matrix: np.ndarray,
                                     original_frame_shape: Tuple[int, int],
                                     tab_name: str = "MAIN_TAB") -> np.ndarray:
    ...
    try:
        ...
        print(f"[{tab_name}] Created unified mask with shape {unified_mask.shape}, {np.sum(unified_mask)} pixels")
        ...
        print(f"[{tab_name}] Applied transformed contour to warped frame: {len(transformed_contour)} points")
        return result_frame
...
def populate_segmentation_model_combo(combo_box, available_models: List[str], 
    ...
    if default_model_path and combo_box.count() > 0:
        for i in range(combo_box.count()):
            ...
            if item_path and default_model_path in str(item_path):
                ...
                print(f"[SEGMENTATION_UTILS] Auto-selected default segmentation model: {combo_box.itemText(i)}")
                break
