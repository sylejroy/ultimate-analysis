...
class JerseyNumberTracker:
    ...
    def _load_config(self) -> None:
        ...
        self.confidence_decay_factor = get_setting("player_id.tracking.confidence_decay_factor", 0.95)
        self.spatial_weight_center_bonus = get_setting("player_id.tracking.spatial_weight_center_bonus", 0.3)
        self.min_confidence_threshold = get_setting("player_id.tracking.min_confidence_threshold", 0.1)
        self.center_region_width = get_setting("player_id.tracking.center_region_width", 0.4)  # 40% of width is "center"
        self.measurement_weight_recent = get_setting("player_id.tracking.measurement_weight_recent", 1.0)
    ...
    def add_measurement(self, track_id: int, jersey_number: str, confidence: float, 
                       bbox_center_x: float, ocr_results: List[Any] = None) -> None:
    ...
    def _update_probabilities(self, track_id: int) -> None:
        ...
        for i, measurement in enumerate(measurements):
            ...
            time_weight = (self.measurement_weight_old + 
                          (self.measurement_weight_recent - self.measurement_weight_old) * age_factor)
            
            # Combined weight
            total_weight = (measurement.confidence * 
                           measurement.spatial_weight * 
                           time_weight)
        ...
        if total_score > 0:
            for jersey_number, score in jersey_scores.items():
                ...
                if jersey_number in probabilities:
                    ...
                    new_prob = (old_prob * (1 - self.probability_smoothing) + 
    ...
    def get_top_probabilities(self, track_id: int, top_k: int = 3) -> List[Tuple[str, float, int]]:
        ...
        for prob_entry in sorted_probs[:top_k]:
            result.append((
                prob_entry.jersey_number,
                prob_entry.probability,
                prob_entry.measurement_count
            ))
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        total_measurements = sum(len(measurements) for measurements in self._track_measurements.values())
...
def add_jersey_measurement(track_id: int, jersey_number: str, confidence: float, 
                          bbox_center_x: float, ocr_results: List[Any] = None) -> None:
